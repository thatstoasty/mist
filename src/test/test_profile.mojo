...
alias ANSI_STYLE = mist.Style(mist.ANSI)
alias ANSI256_STYLE = mist.Style(mist.ANSI256)
alias ASCII_STYLE = mist.Style(mist.ASCII)


def test_ascii_profile_color_conversions():
    alias profile = mist.ASCII_PROFILE
...
def test_ansi_profile_color_conversions():
    alias profile = mist.ANSI_PROFILE
...
def test_ansi256_profile_color_conversions():
    alias profile = mist.ANSI256_PROFILE
...
def test_profile_color():
    ...
    testing.assert_equal(
        String(mist.ANSI256_PROFILE.color(0xC9A0DC)[ANSI256Color].value), String(ANSI256Color(182).value)
    )

    # ANSI profile will degrade the ANSI256 color to the closest ANSI color.
    testing.assert_equal(
        String(mist.ANSI_PROFILE.convert(ANSI256Color(100))[ANSIColor].value), String(ANSIColor(3).value)
    )
...
def test_render_profiles():
    ...
    testing.assert_equal(ASCII_STYLE.foreground(color=mist.ASCII_PROFILE.color(0xC9A0DC)).render(a), "Hello World!")

    # ANSI Color Support (0-15)
    testing.assert_equal(
        ANSI_STYLE.foreground(color=mist.ANSI_PROFILE.color(0xC9A0DC)).render(a),
        "\x1B[95mHello World!\x1B[0m",
    )

    # ANSI256 Color Support (16-255)
    testing.assert_equal(
        ANSI256_STYLE.foreground(color=mist.ANSI256_PROFILE.color(0xC9A0DC)).render(a),
        "\x1B[38;5;182mHello World!\x1B[0m",
    )

    # RGBColor Support (Hex Codes)
    testing.assert_equal(
        TRUE_COLOR_STYLE.foreground(color=mist.TRUE_COLOR_PROFILE.color(0xC9A0DC)).render(a),
