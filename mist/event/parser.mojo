...
fn _is_raw_mode_enabled() -> Bool:
    """Check if raw mode is enabled.

    TODO: Implement actual raw mode detection.
    For now, assume raw mode is enabled.
    """
    return True
...
fn parse_cb(cb: UInt8) raises -> Tuple[MouseEventKind, KeyModifiers]:
    ...
    if button_number == 0 and not dragging:
        kind = MouseDown(MouseButton.Left)
    elif button_number == 1 and not dragging:
        kind = MouseDown(MouseButton.Middle)
    elif button_number == 2 and not dragging:
        kind = MouseDown(MouseButton.Right)
...
fn parse_csi_rxvt_mouse(buffer: Span[UInt8]) raises -> Optional[InternalEvent]:
    ...
    print("parse_csi_rxvt_mouse Buffer", buffer.__str__())
...
fn parse_csi_sgr_mouse(buffer: Span[UInt8]) raises -> Optional[InternalEvent]:
    ...
    print("parse_csi_sgr_mouse Buffer", buffer.__str__())
    ...
    if last_byte == m_BYTE:
        ...
        if kind.isa[MouseDown]():
            var button = kind[MouseDown].button
            kind = MouseUp(button)
...
fn parse_csi_cursor_position(buffer: Span[UInt8]) raises -> Optional[InternalEvent]:
    ...
    print("parse_csi_cursor_position Buffer", buffer.__str__())
    var s = StringSlice(from_utf8=buffer[2 : len(buffer) - 1])
    var parts = s.split(";")
...
fn parse_csi_modifier_key_code(buffer: Span[UInt8]) raises -> Optional[InternalEvent]:
    ...
    assert_starts_with[CSI](buffer)
    print("parse_csi_modifier_key_code Buffer", buffer.__str__())
    var s = StringSlice(from_utf8=buffer[2 : len(buffer) - 1])
    var parts = s.split(";")
    ...
    if key == ord("A"):
        ...
        keycode = FunctionKey(1)
    elif key == ord("Q"):
        keycode = FunctionKey(2)
    elif key == ord("R"):
        keycode = FunctionKey(3)
    elif key == ord("S"):
        keycode = FunctionKey(4)
    else:
        raise could_not_parse_event_error()
...
fn parse_csi_special_key_code(buffer: Span[UInt8]) raises -> Optional[InternalEvent]:
    ...
    print("parse_csi_special_key_code Buffer", buffer.__str__())
    var s = StringSlice(from_utf8=buffer[2 : len(buffer) - 1])
    var parts = s.split(";")
    ...
    if first == 1 or first == 7:
        ...
        keycode = FunctionKey(first - 10)
    elif first >= 17 and first <= 21:
        keycode = FunctionKey(first - 11)
    elif first >= 23 and first <= 26:
        keycode = FunctionKey(first - 12)
    elif first >= 28 and first <= 29:
        keycode = FunctionKey(first - 15)
    elif first >= 31 and first <= 34:
        keycode = FunctionKey(first - 17)
    else:
        raise could_not_parse_event_error()
...
fn parse_csi_u_encoded_key_code(buffer: Span[UInt8]) raises -> Optional[InternalEvent]:
    ...
    print("parse_csi_u_encoded_key_code Buffer:", buffer.__str__())
    var s = StringSlice(from_utf8=buffer[2 : len(buffer) - 1])
    var parts = s.split(";")
