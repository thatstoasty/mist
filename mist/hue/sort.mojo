...
# # distance.
# fn allToAllDistancesCIEDE2000(cs []Color) edgeDistance:
# 	nc = len(cs)
...
# fn sortEdges(m edgeDistance) []edgeIdxs:
...
# fn minSpanTree(nc int, es []edgeIdxs) map[edgeIdxs]struct{:
...
# fn traverseMST(mst map[edgeIdxs]struct{, root int) []int:
